...
class TarefaManager(models.Manager):
    def search(self, query, user):
        return self.dget_queryset().filter(models.Q(nome__icontains=query) | models.Q(descricao__icontains=query) | models.Q(categoria__nome__icontains=query),user=user)
